...
| `create-release-tag.yml` (`Create Release Tag`) | Manual `workflow_dispatch` with a `version` input. | Validates `vMAJOR.MINOR.PATCH`, verifies `RELEASE_TAG_APPROVER`, and rejects existing tags. | Creates an annotated `v*` release tag from the current `main` commit using `RELEASE_TAG_DEPLOY_KEY` with persisted checkout credentials disabled. |
