...
| `release.yml` (`Release Public Deploy`) | Pushes to `main` and manual `workflow_dispatch` runs. Manual runs can choose a patch, minor, or major public deploy version bump and can force a release when no files changed. | Checks the public release note generators, generated CI env values, and release metadata consistency before publishing. | Publishes the next independent `public-deploy-vX.Y.Z` GitHub Release when repository files changed since the previous public deploy tag, including automated source app contract update commits. Uploads `release-manifest.json`, `release-verification.json`, and the current immutable `releases/<source-app-tag>.json` public contract. |

These workflows use pinned third-party actions where configured and Node.js 24 for
the repository's JavaScript validation helpers. Public deploy release tags are
intentionally separate from source application tags such as `v0.2.4`.
