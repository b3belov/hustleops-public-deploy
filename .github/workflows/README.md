...
| `update-from-contract.yml` (`Update From Release Contract`) | Manual `workflow_dispatch` with a `contract-ref` input. | Checks the release contract OCI digest and cosign signature against the pinned GitHub Actions issuer and certificate identity pattern, validates contract schema/trust/runtime/image facts through `update-from-contract.mjs`, verifies runtime image signatures, then repeats script syntax, ShellCheck, Node syntax, `actionlint`, release metadata, Compose, Nginx, preflight, and deploy dry-run checks. | Pulls a verified release contract, updates repo release artifacts, pushes an automation branch with `PUBLIC_DEPLOY_UPDATE_DEPLOY_KEY`, and opens or updates the public deploy update PR when generated files changed. |
| `create-release-tag.yml` (`Create Release Tag`) | Manual `workflow_dispatch` with a `version` input. | Validates `vMAJOR.MINOR.PATCH`, verifies `RELEASE_TAG_APPROVER`, verifies release-tag GitHub App credentials, and rejects existing tags. | Creates an annotated `v*` release tag from the current `main` commit using a dedicated GitHub App installation token with persisted checkout credentials disabled. |
| `release.yml` (`Release`) | Pushes to `v*` tags and manual `workflow_dispatch` with a `version` input. | Verifies the tag commit is reachable from `origin/main`, validates release metadata, builds release notes, and checks `PRODUCTION_RELEASE_APPROVER` for manual publishes. | Builds on tag push, and publishes the protected GitHub Release only through manual dispatch after source verification, build validation, explicit approver verification, and `production` environment approval. |
| `deploy.yml` (`Deploy App`) | Manual `workflow_dispatch` with `release_ref` and `env_file` inputs. | Validates the selected protected source, runs the deploy-bundle validation surface, verifies `PRODUCTION_DEPLOYMENT_APPROVER`, and waits on the `production` environment. | Deploys with `scripts/deploy.sh update` only from a production-labeled self-hosted runner. |
...
host Docker context, persistent volumes, or production `.env` file.
