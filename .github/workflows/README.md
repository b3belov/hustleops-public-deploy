...
| `create-release-tag.yml` (`Create Release Tag`) | Manual `workflow_dispatch` with a `version` input. | Validates `vMAJOR.MINOR.PATCH`, verifies release-tag GitHub App credentials, and rejects existing tags. | Creates an annotated `v*` release tag from the current `main` commit using a dedicated GitHub App installation token with persisted checkout credentials disabled. |
| `release.yml` (`Release`) | Pushes to `v*` tags. | Verifies the tag commit is reachable from `origin/main`, validates release metadata, builds release notes, and keeps release publication behind the verified tag and successful build. | Publishes the protected GitHub Release automatically after tag source verification and build validation succeed. |
